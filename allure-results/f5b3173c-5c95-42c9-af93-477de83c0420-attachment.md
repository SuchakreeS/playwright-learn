...
Locator:  getByText('United States')
Expected: checked
Received: unchecked
Timeout:  30000ms
...
Call log:
  ...
  - waiting for getByText('United States')
...
```ts
  ...
  11 |     await page.getByText('Japan').check();
  12 |     await expect(page.getByText('Japan')).toBeChecked();
> 13 |     await expect(page.getByText('United States')).toBeChecked();
     |                                                   ^ Error: expect(locator).toBeChecked() failed
  14 |     await page.getByText('Playwright', { exact: true }).click();
  15 |     await page.getByText('JavaScript', { exact: true }).click();
  16 |     await expect(page.getByText('Playwright', { exact: true })).toBeVisible();
  17 | })
```